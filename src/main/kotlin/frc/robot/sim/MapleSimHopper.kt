...
class MapleSimHopper {
    ...
    private fun createLayers(numberOfLayers: Int, addForwardRows: Boolean = false): List<Pose3d> =
        ((0 until numberOfLayers).flatMap { layer ->
            val heightOffset = (0.2 + 0.15 * layer).m

            val baseBalls = BALL_POSES_RELATIVE_TO_ROBOT.map {
                (it.toPose().toPose3d() +
                        Transform3d(
                            drive.pose.translation.toTranslation3d(heightOffset),
                            Rotation3d()
                        )).rotateAround(drive.pose.translation.toTranslation3d(), drive.pose.rotation.toRotation3d())
            }

            if (addForwardRows) {
                val forwardRows = listOf(28, 43, 57).flatMap { x ->
                    (-30..30 step 15).map { y ->
