...
object Flywheel : SubsystemBase(), FlywheelVelocitiesCommandFactory {
    ...
    private val motors = mutableListOf<UniversalTalonFX>(motor)
    ...
    init {
        AUXILIARY_MOTORS_PORTS.forEach {
            ...
            aux.setControl(Follower(MAIN_MOTOR_PORT, it.value))
            motors.add(aux)
        }
    }
    ...
    fun setCurrentLimits(limits: CurrentLimitsConfigs): Command =
        Commands.runOnce({
            motors.forEach {
                it.applyConfiguration(MOTOR_CONFIG.withCurrentLimits(limits))
            }
        })
