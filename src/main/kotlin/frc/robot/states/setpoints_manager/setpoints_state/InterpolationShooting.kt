...
private fun getHoodSetpoint(): Angle {
    val hoodKey = InterpolatingDouble(drive.pose.distanceFromPoint(currentGoal)[m])
    return SHOOTER_ANGLE_BY_DISTANCE.getInterpolated(hoodKey).value.degrees
}

private fun getFlywheelSetpoint(): AngularVelocity {
    val flywheelKey = InterpolatingDouble(drive.pose.distanceFromPoint(currentGoal)[m])
...
private fun getPreShooterSetpoint(): AngularVelocity {
    val preShooterKey = InterpolatingDouble(drive.pose.distanceFromPoint(currentGoal)[m])
    return SHOOTER_VELOCITY_BY_DISTANCE.getInterpolated(preShooterKey).value.rps
}

val interpolationShootingMap: Map<SubsystemBase, () -> Measure<out Unit>> = mapOf(
